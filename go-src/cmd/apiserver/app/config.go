...
const (
	defaultHomeDir    = ".server"
	defaultConfigName = "server-apiserver.yaml"
)
...
func onInitialize() {
	...
	if err := viper.ReadInConfig(); err != nil {
		log.Fatalw("Failed to read viper configuration file, err: %v", err)
	}

	// 打印当前使用的配置文件，方便调试
	log.Debugw("Using config file: %s", viper.ConfigFileUsed())
}
...
func setupEnvironmentVariables() {
	...
	viper.AutomaticEnv()
	// 设置环境变量前缀
	viper.SetEnvPrefix("FFMPEG_AUDIO")
